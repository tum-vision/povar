...
# lookup executable to run: HERE I HAVE CHANGED bal TO bal_sc
EXECUTABLE=`"$SCRIPT_DIR"/query-config.py rootba_config.toml batch_run.executable bal`

CMD="$ROOTBA_BIN_DIR/$EXECUTABLE"
...
for i in $(seq $NUM_RUNS); do
    echo ">>> Run $i" 2>&1 | tee -a output.log
    { $TIMECMD "$CMD"; } 2>&1 | tee -a output.log
