...
class Plot(ExperimentsContainer):
    ...
    def plot_bal_bar_one(self, exps, spec, ax1, tol):
        ...
        # always_staged_solvers = ["bal_implicit_qr", "bal_implicit_qr2", "bal_factor_qr"]
        ...
        # preprocess time is LMB allocation, factor grouping, etc before LM iterations start
        preprocess = np.array(
        ...
        for i, (l, name) in enumerate(zip(logs, names)):
            ...
            if l is not None:
                ...
                if ax6 is not None:
                    ...
                    if l._static.solver.fraction_grouped:
                        kwargs["label"] += " " + str(int(l._static.solver.fraction_grouped * 100)) + "%"
